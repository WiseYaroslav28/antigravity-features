...
const initLanguageSwitcher = () => {
    if (window.__antigravity_translation_initialized) return;
...
const _anti_initInterval = setInterval(() => {
    if (document.body && document.head) {
        clearInterval(_anti_initInterval);
        const protocol = window.location.protocol;
        const hostname = window.location.hostname;
        const isInternal = ['vscode-file:', 'vscode-webview:', 'antigravity-ide:'].includes(protocol) || ['127.0.0.1', 'localhost'].includes(hostname);
        if (isInternal) {
            initLanguageSwitcher();
        }
    }
