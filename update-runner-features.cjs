...
async function runUpdate() {
  try {
    ...
    // Принудительно завершаем родительский процесс
    if (parentPid && parentPid > 0) {
      try {
        process.kill(parentPid, 'SIGKILL');
        writeLog(`Родительский процесс ${parentPid} успешно остановлен.`);
      } catch (killErr) {
        writeLog(`Предупреждение при убийстве родителя: ${killErr.message}`);
      }
    }
